...
Now, we're down to just **`r updatedlabelcount2`** EVTYPE labels, all of which fit the reporting guidelines.  Let's use a similar process to address potentially bad data in PROPDMGEXP and CROPDMGEXP fields.
...
# add an integer column for total econ damage (takes a while)
mutate(storm.data.replaced2, total.econ.damage = 0)
...
f <- function(df) {
  ...
  if(df["PROPDMGEXP"] == "K"){
    propdamage <- df["PROPDMG"] * 1000
  }else if(df["PROPDMGEXP"] == "M"){
    propdamage <- df["PROPDMG"] * 1000000
  }else if(df["PROPDMGEXP"] =="B"){
    propdamage <- df["PROPDMG"] * 1000000000
  }else{
    propdamage <- df["PROPDMG"]
  }
  
  # convert column values to integers
  if(df["CROPDMGEXP"] == "K"){
    cropdamage <- df["CROPDMG"] * 1000
  }else if(df["CROPDMGEXP"] == "M"){
    cropdamage <- df["CROPDMG"] * 1000000
  }else if(df["CROPDMGEXP"] =="B"){
    cropdamage <- df["CROPDMG"] * 1000000000
  }else{
    cropdamage <- df["CROPDMG"]
  }
  
  return(propdamage + cropdamage)
}

# run this function on the whole data frame
o <- apply(storm.data.replaced2, 1, f)
```
